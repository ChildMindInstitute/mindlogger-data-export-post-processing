...
def test_date_processor(field_name, timestamps, expected):
    """Test DateTimeProcessor."""
    preprocessor = DateTimeProcessor()
    report = pl.DataFrame({field_name: timestamps, "utc_timezone_offset": [0, 0]})
    processed_report = preprocessor._run(report)
    assert processed_report[field_name].to_list() == expected
...
def test_response_processor_single_row(response_field, expected):
    ...
    processed_expected = {
        "status": "completed",
        "raw_score": None,
        "value": RESPONSE_VALUE_DICT_SCHEMA | expected,
    }
    ...
    processed_report = preprocessor._run(report)
    assert processed_report["response"].to_list().pop() == processed_expected
...
def test_response_processor_multi_row(responses, expected):
    """Test ResponsePreprocessor on data with multiple rows."""
    preprocessor = ResponseStructProcessor()
    expected = [RESPONSE_VALUE_DICT_SCHEMA | e for e in expected]
    report = pl.DataFrame(
        {
            "item_response": responses,
            "item_response_status": ["completed"] * len(responses),
            "rawScore": [None] * len(responses),
        },
    )
    processed_report = preprocessor._run(report)
    assert processed_report["response"].to_list() == [
        {"status": "completed", "value": e, "raw_score": None} for e in expected
    ]
...
# Struct([
#   Field { name: "raw_value", dtype: String },
#   Field { name: "null_value", dtype: Boolean },
#   Field { name: "single_value", dtype: Int64 },
#   Field { name: "value", dtype: List(Int64) },
#   Field { name: "text", dtype: String },
#   Field { name: "file", dtype: String },
#   Field { name: "date", dtype: Date },
#   Field { name: "time", dtype: Time },
#   Field { name: "time_range", dtype: Duration(Microseconds) },
#   Field { name: "geo", dtype: Struct([ Field { name: "latitude", dtype: Float64 }, Field { name: "longitude", dtype: Float64 }]) },
#   Field { name: "matrix", dtype: List(Struct([ Field { name: "row", dtype: String }, Field { name: "value", dtype: List(String) }])) },
# got 'Struct([
#   Field { name: "type", dtype: String },
#   Field { name: "type", dtype: String },
#   Field { name: "raw_value", dtype: String },
#   Field { name: "null_value", dtype: Boolean },
#   Field { name: "single_value", dtype: Null },
#   Field { name: "value", dtype: List(String) },
#   Field { name: "text", dtype: String },
#   Field { name: "file", dtype: String },
#   Field { name: "date", dtype: Date },
#   Field { name: "time", dtype: Time },
...
#   Field { name: "geo", dtype: Struct([ Field { name: "latitude", dtype: Float64 }, Field { name: "longitude", dtype: Float64 }]) },
#   Field { name: "matrix", dtype: List(Struct([ Field { name: "row", dtype: String }, Field { name: "value", dtype: List(String) }])) },
#   Field { name: "optional_text", dtype: String },
#   Field { name: "optional_text", dtype: Null },
#   Field { name: "subscale", dtype: Float64 }])',
#   Field { name: "subscale", dtype: Null }])';
#
...
def test_subscale_processor():
    ...
    report = pl.DataFrame(
        {
            ...
            "item_type": ["it_1", "it_2"],
            "item_response": ["r_1", "r_2"],
            "item_prompt": ["prompt_1", "prompt_2"],
            "item_response_options": ["options_1", "options_2"],
            "item_response_status": ["completed", "completed"],
            ...
            "activity_score": [1, 5],
            "activity_score_lookup_text": ["t_1", "t_2"],
            "subscale_avg": [2, 6],
            "subscale_text_avg": ["t_3", "t_4"],
        }
    )
    preprocessor = SubscaleProcessor()
    processed_report = preprocessor._run(report)
    assert "activity_score" not in processed_report.columns
    assert "activity_score_lookup_text" not in processed_report.columns
    assert "subscale_avg" not in processed_report.columns
    assert "subscale_text_avg" not in processed_report.columns
    ...
    assert_frame_equal(
        processed_report.select("activity_submission_id", "item_name", "item_response"),
        expected,
